/**
 * AllReduce configurable parameter test (CPU wall-clock timing)
 *
 * Features:
 ...
 *   ./build.sh allreduce_param_wall.cc hccl_allreduce_param_wall
 */
...
// Simple CPU wall-clock timer, millisecond granularity
class HostTimer {
private:
...
int main(int argc, char* argv[]) {
    ...
    printf("AllReduce param (wall timer) test: devices=%u send_count=%lu iters=%u (%.2f MB per rank per iter)\n",
           devCount, (unsigned long)sendCount, numIters,
